    ...
    if (config.almond === true) {
      ...
      if (!util._.isArray(moduleIterator)) {
        ...
        if (!util._.isArray(configClone.include)) {
          ...
          configClone.include.unshift('almond');
        }

        // check if the app is named, else assign ´main´ as name
        // dirty workaround for lazy people like me
        if (!configClone.name) {
          configClone.name = 'main';
        }

        moduleIterator = [{name: configClone.name}];
      }
